...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./EzilaXBotV/resources/animelogo.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "white"
    shadowcolor = "blue"
    font = ImageFont.truetype("./EzilaXBotV/resources/HalloweenNight-vmyPD.ttf", 1000)
    w, h = draw.textsize(text, font=font)
    h += int(h*0.21)
    image_width, image_height = img.size
    draw.text(((image_widthz-w)/2, (image_heightz-h)/2), text, font=font, fill=(255, 255, 255))
    x = (image_widthz-w)/2
    y= ((image_heightz-h)/2+6)
    draw.text((x, y), text, font=font, fill="yellow", stroke_width=0, stroke_fill="red")
    fname2 = "EzilaXBotVLogo.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @EzilaXBot ❤️")
    if os.path.exists(fname2):
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./EzilaXBotV/resources/boylogo.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "white"
...
@EzilaXBot
 ...
 ❍ /animelogo text :  Create your logo with your name
 ❍ /boylogo text :  Create your logo with your name
 """
